...
def graph_search(
    ...
    while not frontier.is_empty():
        ...
        if goal_description.is_goal(next_state):
            num_generated, elapsed_time = print_search_status(expanded, frontier)
            return True, next_state.extract_plan(), num_generated, elapsed_time
        
        expanded.add(next_state)

        for action in next_state.get_applicable_actions(action_set):
            child = next_state.result(action)
            child.parent = next_state
            child.path_cost = next_state.path_cost + 1
